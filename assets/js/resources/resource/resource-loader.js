...
async function read_metadata() {
    const response = await fetch('/metadata/resources/meta.json')
...
async function read_markdown(name) {
    const response = await fetch('/metadata/resources/md/' + name + '.md')
...
async function load_resource(){
    var resource_name = document.getElementById("resource-name")
    resource_name.textContent = "Not found"
    if(
        params.has('id')
    ){
        ...
        data.list.forEach(item => {
            if(item.link == id){
                resource_name.textContent = item.name;
            }
        })
